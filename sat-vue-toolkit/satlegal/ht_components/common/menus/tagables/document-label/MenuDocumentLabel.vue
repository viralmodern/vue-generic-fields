<template>
  <div>
    <MenuLabel
      ...
      :positionX="positionX"
      :positionY="positionY"
      :listLabels="dataDocumentLabel"
      :labels="labels"
      title="List Document Label"
      keyText="name"
      keyId="pk"
      keyColor="color"
      nameAdd="Add new"
...
import baseDocumentLabelMixin from "./mixins/baseDocumentLabelMixin";
...
export default {
  ...
  methods: {
    async add({ dataForm, closeAdd }) {
      ...
      console.log("dataFormMap", dataFormMap)
      try {
        this.isLoadingAdd = true;
        await this.addDocumentLabelRequest({
    ...
    async update({ dataForm, closeUpdate }) {
      try {
        ...
        await this.updateDocumentLabelRequest({
          matterId: this.$route.params.matter || this.$route.params.projectId,
          ...dataForm
        });
    ...
    removeLabel(id) {
      ...
      this.removeDocumentLabelRequest({ matterId: this.$route.params.matter || this.$route.params.projectId, id })
    }
  }
};
</script>
