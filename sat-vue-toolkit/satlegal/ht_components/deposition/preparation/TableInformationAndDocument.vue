  ...
  <div class="px-2">
    ...
    <div class="py-2" v-for="stage in item.list" :key="stage.name">
      ...
      <SlickTable
        ref="refSlickTable"
        class="py-2"
        :totals="(dataInfoAndDoc[stage.value] || {}).totals"
        :items="(dataInfoAndDoc[stage.value] || {}).list"
      ...
      >
        <template slot="cell" slot-scope="props">
          <component
            ...
            "
            @remove="removeInfoAndDoc(stage.name, props.dataRow, $event)"
          />
        </template>
        <template v-slot:top>
    ...
    <FormCreateInfoAndDoc
      ref="refFormCreateInfoAndDoc"
      :isLoadingAdd="isLoadingAdd"
      :isShowPopup="isShowCreate"
      header="Add new information and document"
      :stageAdd="{ field: this.item.field, ...this.stageAdd }"
      @closeModal="isShowCreate = false"
      @submit="addInfoAndDoc"
    />
  </div>
</template>
...
import FormCreateInfoAndDoc from "../../dialogs/FormCreateInfoAndDoc";
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("ht_store/matter/discovery/deposition/preparation/infoAndDoc", [
      "dataInfoAndDoc", "isLoadingGet"
    ]),
  ...
  methods: {
    ...
    async addInfoAndDoc(dataForm) {
      try {
        ...
        await this.addInfoAndDocRequest(data);
        this.$refs.refFormCreateInfoAndDoc.clear();
    ...
    updateInfoAndDoc(item, infoAndDocId, name, value) {
      const data = {
        name: item,
        idParams: {
          matterId: this.dataDepositionDetail.matter,
      ...
      this.updateInfoAndDocRequest(data);
    },
    async updateInfoAndDocAsync(item, infoAndDocId, name, {value, setLoading}) {
      try {
        const data = {
          name: item,
          idParams: {
        ...
        setLoading(true);
        await this.updateInfoAndDocAsyncRequest(data);
        setLoading(false);
      } catch (error) {
        setLoading(false);
      }
    },
    async removeInfoAndDoc(name, row, close) {
      const data = {
        name,
        matterId: this.dataDepositionDetail.matter,
        depositionId: this.dataDepositionDetail.pk,
      ...
      await this.removeInfoAndDocRequest(data);
      close();
    },
    updatePagination({ page, ungroup, group_by, group_items }) {
      if (this.panel === this.index) {
        ...
        this.getDataInfoAndDocRequest({ idParams, searchObject });
      }
    },
    onEndDragRows(event) {
      const {
      ...
      let data = {
        ...
        idParams: {
          matterId: this.dataDepositionDetail.matter,
          depositionId: this.dataDepositionDetail.pk,
          infoAndDocId: drag.taskId
        }
      };
      const fromTask = this.dataInfoAndDoc[fromListId].list.find(
        x => x.id == taskId
      )[this.item.field];
    ...
    ...mapMutations("ht_store/matter/discovery/deposition/preparation/infoAndDoc", [
      "clearData"
    ]),
    ...mapActions("ht_store/matter/discovery/deposition/preparation/infoAndDoc", [
      "getDataInfoAndDocRequest",
      "addInfoAndDocRequest",
      "updateInfoAndDocRequest",
      "updateInfoAndDocAsyncRequest",
      "removeInfoAndDocRequest"
    ])
  },
  mounted() {
