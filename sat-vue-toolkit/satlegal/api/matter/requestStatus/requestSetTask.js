...
async function getDataRequestSetTask({ idParams, searchObject = {} }) {
  const url = `/api/matters/${idParams.matterId}/request-sets/all-request-tasks/`;
  const options = {
...
async function getRequestSetTaskDetail(idParams) {
  const url = `/api/matters/${idParams.matterId}/request-sets/${idParams.rSId}/tasks/${idParams.pk}/`;
...
async function addRequestSetTask({ idParams, body }) {
  const bodyMap = {
    ...cloneDeep(body),
    litigation: getFieldNullAble(body.litigation),
    activities: getFieldNullAble(body.activities),
    status: getFieldNullAble(body.status),
    request_set: getFieldNullAble(body.request_set),
    request_item_id: getFieldNullAble(body.request_item_id),
  };
...
async function updateRequestSetTask({ idParams, body }) {
  ...
  if (bodyMap.request_items) {
    bodyMap.request_item_id = bodyMap.request_items[bodyMap.request_items.length - 1].pk;
    delete bodyMap.request_items;
  }
...
async function removeRequestItemInTask(data) {
  const url = `/api/matters/${data.matterId}/request-sets/${data.requestSetId}/tasks/${data.requestSetTaskId}/request-items/${data.requestItemId}/`;
  const result = await remove(url);
  return result;
}
