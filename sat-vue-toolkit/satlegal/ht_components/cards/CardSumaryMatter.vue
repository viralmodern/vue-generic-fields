<template>
  <v-card>
          ...
          <v-row no-gutters class="ma-0" align-center>
            ...
            <v-col cols="10">
              <div class="pl-3">
                ...
                <v-row no-gutters>
                  <v-col class="text-caption">{{ formatTimeCreator }}</v-col>
                </v-row>
              </div>
            </v-col>
          </v-row>
    ...
    <v-divider></v-divider>
    <div class="px-3 py-1 font-weight-bold subheading">{{ item.title }}</div>
...
export default {
  ...
  computed: {
    formatTimeCreator() {
      return moment(this.item.startDate).fromNow();
    },
    dataFor() {
