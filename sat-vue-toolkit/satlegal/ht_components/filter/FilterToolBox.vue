...
export default {
  ...
  computed: {
    ...
    ...mapGetters("ht_store/matter/discovery/toolbox", ["hasFilter"]) // eslint-disable-line
  },
  methods: {
    filterRequest(name, value) {
      this[name] = value;
      this.filterToolboxRequest({ name, value });
    },
    close() {
      this.openRight("filterToolbox");
    },
    ...mapMutations("ht_store/layout", ["openRight"]), // eslint-disable-line
    ...mapActions("ht_store/matter/discovery/toolbox", ["filterToolboxRequest"]) // eslint-disable-line
  }
};
</script>
