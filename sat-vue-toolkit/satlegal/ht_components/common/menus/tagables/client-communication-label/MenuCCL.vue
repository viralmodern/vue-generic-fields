<template>
  <div>
    <MenuLabel
      ...
      :positionX="positionX"
      :positionY="positionY"
      :listLabels="dataCommunicationLabel"
      :labels="labels"
      :title="title"
      keyText="name"
      keyId="pk"
...
import baseCCLMixin from "./mixins/baseCCLMixin";
  ...
  methods: {
    async add({ dataForm, closeAdd }) {
      ...
      try {
        ...
        await this.addCommunicationLabelRequest({
    ...
    async update({ dataForm, closeUpdate }) {
      try {
        ...
        await this.updateCommunicationLabelRequest({
          matterId: this.$route.params.matter || this.$route.params.projectId,
          ...dataForm
        });
    ...
    removeLabel(id) {
      ...
      this.removeCommunicationLabelRequest({ matterId: this.$route.params.matter || this.$route.params.projectId, id })
    }
  }
};
</script>
