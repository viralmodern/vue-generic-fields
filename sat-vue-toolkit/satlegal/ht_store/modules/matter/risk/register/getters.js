...
export default {
  dataRisk: state => {
    const cloneDataRisk = cloneDeep(state.dataRisk);
    for (const key in cloneDataRisk) {
      cloneDataRisk[key].results.forEach(x => {
    ...
    return cloneDataRisk;
  },
  dataAllRisk: state => {
    return state.dataAllRisk.map(x => {
      return {
        id: x.pk,
  ...
  dataAllRiskNoGroup: state => state.dataAllRiskNoGroup,
  isLoadingGet: state => state.isLoadingGet,
  dataAttachments: state => state.dataAttachments,
  riskDetail: state => state.riskDetail,
