...
<template>
  <div class="gantt-elastic" style="width:100%">
    ...
    <main-view :tasks="tasks" :options="options" style="background: none;">
...
function getOptions(userOptions) {
  ...
  if (
    ...
    typeof userOptions.locale.name === "string"
  ) {
...
const GanttElastic = {
  ...
  methods: {
    ...
    getScrollBarHeight() {
      ...
      return this.$store.commit(this.updateOptionsMut, {
        scrollBarHeight: noScroll - withScroll
      });
    },
      ...
      mergeWith.forEach(objOrClassName => {
        if (typeof objOrClassName === "string") {
          merged = Object.assign(
            {},
            merged,
            this.internalOptions.style[objOrClassName]
          );
    ...
    initialize(itsUpdate = "") {
      ...
      let options;
      if (itsUpdate === "") {
        options = this.mergeDeepReactive(
          {},
      ...
      if (itsUpdate === "" || itsUpdate === "options") {
        ...
        this.$store.commit(this.updateOptionsMut, {
          outerHeight:
            this.internalOptions.height + this.internalOptions.scrollBarHeight
        });
      }
    ...
    calculateCalendarDimensions() {
      let height = 0;
      if (
        this.internalOptions.calendar.hour.display &&
        this.internalOptions.calendar.hours.length > 0
      ) {
        height += this.internalOptions.calendar.hour.height;
      }
      if (
        this.internalOptions.calendar.day.display &&
        this.internalOptions.calendar.days.length > 0
      ) {
        height += this.internalOptions.calendar.day.height;
      }
      if (
        this.internalOptions.calendar.month.display &&
        this.internalOptions.calendar.months.length > 0
      ) {
    ...
    getMaximalExpanderWidth() {
      return (
        this.getMaximalLevel() *
          this.internalOptions.taskList.expander.padding +
      ...
      this.internalOptions.taskList.columns.forEach(column => {
        if (column.expander) {
          column.widthFromPercentage =
            ...
            this.internalOptions.taskList.percent;
        } else {
          column.widthFromPercentage =
            (column.width / 100) * this.internalOptions.taskList.percent;
        }
      ...
      tasks.forEach(current => {
        if (current.parentId === parent.id) {
          ...
          current.allChildren.forEach(childId =>
            parent.allChildren.push(childId)
          );
        }
      });
      return parent;
    ...
    getTask(taskId) {
      for (
        let i = 0, len = this.$store.state.GanttElastic.tasks.length;
        i < len;
        i++
      ) {
     ...
     */
    getChildren(taskId) {
      return this.$store.state.GanttElastic.tasks.filter(
        task => task.parent === taskId
      );
    },
    ...
    pixelOffsetXToTime(pixelOffsetX) {
      ...
      return (
        offset * this.internalOptions.times.timePerPixel +
        this.internalOptions.times.firstTime
      );
    },
     ...
     */
    onScrollChart() {
      const horizontal = this.internalOptions.refs
    ...
    _onScrollChart(left, top) {
      const chartContainerWidth = this.internalOptions.refs.chartContainer
        .clientWidth;
      const options = {
        scroll: {
          chart: {
            ...
            percent:
              (left / this.internalOptions.times.totalViewDurationPx) * 100,
            top: top,
            ...
            dateTime: {
              ...
              right: this.pixelOffsetXToTime(
                left + this.internalOptions.refs.chart.clientWidth
              )
            }
          }
        }
    ...
    onWheelChart(ev) {
      if (!ev.shiftKey) {
        ...
        const scrollHeight =
          this.internalOptions.refs.chartGraph.scrollHeight - chartClientHeight;
        if (top < 0) {
          top = 0;
        ...
        const chartClientWidth = this.internalOptions.refs
          .chartScrollContainerHorizontal.clientWidth;
        const scrollWidth =
          this.internalOptions.refs.chartScrollContainerHorizontal.scrollWidth -
          chartClientWidth;
        if (left < 0) {
          left = 0;
    ...
    onScopeChange(value) {
      this.$store.commit(this.updateOptionsMut, {
        scope: { before: value, after: value }
      });
      this.initTimes();
      this.calculateSteps();
    ...
    recalculateTimes() {
      ...
      const timePerPixel =
        this.internalOptions.times.timeScale * steps * percent +
        Math.pow(2, this.internalOptions.times.timeZoom);
      const totalViewDurationMs = dayjs(
        this.internalOptions.times.lastTime
      ).diff(dayjs(this.internalOptions.times.firstTime), "milisecods");
      ...
      this.$store.commit(this.updateOptionsMut, {
        ...
        width:
          totalViewDurationPx + this.style("grid-line-vertical")["stroke-width"]
      });
    },
    ...
    computeDayWidths() {
      ...
      for (
        let day = 0, daysLen = options.times.steps.length;
        day < daysLen;
        day++
      ) {
        const widths = {
    ...
    computeMonthWidths() {
      ...
      const monthsCount = Math.ceil(
        dayjs(this.internalOptions.times.lastTime).diff(
    ...
    prepareDates() {
      ...
      let lastTaskTime = 0;
      for (
        let index = 0, len = this.$store.state.GanttElastic.tasks.length;
        index < len;
        index++
      ) {
    ...
    globalOnResize() {
      ...
      if (
        ...
        (this.internalOptions.clientWidth / 100) *
          this.internalOptions.taskList.widthThreshold
      ) {
        const diff =
          this.internalOptions.taskList.widthFromPercentage -
          (this.internalOptions.clientWidth / 100) *
        ...
        let diffPercent =
          100 -
          (diff / this.internalOptions.taskList.widthFromPercentage) * 100;
        if (diffPercent < 0) {
          diffPercent = 0;
        }
        options.taskList.columns = this.internalOptions.taskList.columns.map(
          column => {
  ...
  computed: {
     ...
     */
    visibleTasks() {
      const visibleTasks = this.internalTasks.filter(task =>
      ...
      if (
        this.internalOptions.maxHeight &&
      ...
      for (let index = 0; index < len; index++) {
        ...
        props.width =
          task.durationMs / this.internalOptions.times.timePerPixel -
  ...
  created() {
    ...
    this.$watch("tasks", tasks => {
      const oldTasks = this.$store.state.GanttElastic.tasks.map(
        (task, index) => {
    ...
    this.$watch("internalOptions", () => {
      const newOptions = synchronizeDeep(
