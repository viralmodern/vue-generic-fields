<template>
  <td>
    <a href="#" @click="showDialog=true">{{dataRow.receipt.length}} receipt(s)</a>
...
export default {
  ...
  methods: {
    ...
    ...mapActions("ht_store/matter/briefcase/billing/expense/receipt", [ // eslint-disable-line
  ...
  computed: {
    getAttachment() {
      return this.dataAttachment.filter(x => {
        return this.dataRow.receipt.includes(x.id);
      });
    },
    ...mapGetters("ht_store/matter/briefcase/billing/expense/receipt", ["dataAttachment"]) // eslint-disable-line
  }
};
</script>
