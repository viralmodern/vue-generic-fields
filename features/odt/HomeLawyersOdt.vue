...
let { CompanyProfile, Education, LawyerProfile, PracticeAreas } = ROUTES.Lawyers
export default {
  name: 'HomeLawyersOdt',
  props: {
    height: {
  ...
  components: {
    ...
    GridAllLawyersOdt: () => import('~/features/odt/GridAllLawyersOdt'),
  },
  computed: {
    reduceIncludesSubs() {
    ...
    subsLawyers() {
      ...
      let tabs = [
        ...
        { ...generateToSub(PracticeAreas, query) },
        { ...generateToSub(Education, query) },
        { ...generateToSub(LawyerProfile, query) },
      ]
