        ...
        <v-row class="ma-0">
          ...
          <v-col cols="6" class="pa-0 pr-2 mt-2">
            <div class="d-flex ml-2">
              <Label class="mr-2" :value="labels" @update="labels = $event" />
              <DatePicker
                title="Choose date"
                ...
                :isClear="true"
              />
              <v-btn small class="primary mx-1" @click="validForm" :loading="isLoadingAdd">Add</v-btn>
            </div>
          </v-col>
        </v-row>
      </v-form>
...
export default {
  ...
  methods: {
    ...
    async add() {
      if (this.$refs.form.validate() && this.validate === false) {
        const data = {
          ...
          body: {
            able_transcript: false,
            desc: this.desc,
            deposition_date: this.date,
            case: this.case_name,
            labels: this.labels
          }
        };
    ...
    ...mapActions("ht_store/matter/deposition/givenDeposition", ["addRequestApi"])
  },
  watch: {
    groupBy(val, oldVal) {
