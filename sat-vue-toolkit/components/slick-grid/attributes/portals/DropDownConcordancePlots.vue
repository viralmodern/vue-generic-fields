<template>
  <v-menu
    ...
    <v-card tile :loading="loading" :disabled="loading" width="400px">
      <v-card-text>
        <PhraseConcordancePlots v-if="document && !loading"
                                :phrase="phrase"
...
<script>
  ...
  export default {
    name: "DropDownConcordancePlots",
    components: { PhraseConcordancePlots },
    data() {
      return {
        showMenu: false,
        x: 0,
      ...
      show(e) {
        ...
        this.$nextTick(async () => {
          if(!this.disabledDialogPhrase) {
            ...
            this.$trigger('concordancePlotsCell', { phrase: this.phrase, rq: this.fetchDocumentDetail })
          }
        })
      },
