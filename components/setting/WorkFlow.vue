<template>
  <v-card>
    <v-card-text>
      <div class="text-h6 mb-3">Matter Procedures</div>
      <v-btn
        fab
        dark
      ...
      />
      <v-col v-for="(val, index) in dataWorkFlow" :key="index" class="my-2">
        <ItemWorkFlow
          :item="val"
...
export default {
  ...
  methods: {
    ...mapActions('adminSetting/workFlow', ['getDataWorkFlowRequest']),
    ...mapMutations('adminSetting/workFlow', [
      'deleteStepByParent',
      'addWorkFlowStep',
      'editWorkFlowStep',
      'deleteWorkFlow',
