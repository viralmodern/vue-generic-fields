<template>
  <v-container fluid>
          ...
          >
            <template v-slot:item.request_item="{ props: { item } }">
              <TextInline
                ...
                $_toggleMenuRequestSetItemMixin_openMenu({
                  ...
                  itemsId = item.request_item && item.request_item.pk
              "
                :isReadOnly="true"
              />
            </template>
            <template
              v-slot:item.number="{ props: { item } }"
            >{{item.request_item && item.request_item.pk }}</template>
            <template v-slot:item.name="{ props: { item } }">
              <TextField
                :value="item.name"
                @update="updateRSID(item.id, 'name', $event, item.request_item && item.request_item.pk)"
              />
            </template>
            <template v-slot:item.date="{ props: { item } }">
              <v-row no-gutters class="td-date">
                ...
                "
                  ...
                  class="mx-2"
                  @updateTime="updateRSID(item.id, 'date', $event.date, item.request_item && item.request_item.pk)"
                />
              </v-row>
            </template>
            <template v-slot:item.attachment="{ props: { item } }">
              <AttachmentLink
                ...
                @update="updateAttachmentRSID(item.id, $event.value, item.request_item && item.request_item.pk)"
              />
            </template>
            <template v-slot:item.custodians="{ props: { item } }">
              <RelatedCharacter
                :value="item.custodians"
                @update="updateRSID(item.id, 'custodians', $event, item.request_item && item.request_item.pk)"
              />
            </template>
            <template v-slot:item.recipients="{ props: { item } }">
              <RelatedCharacter
                :value="item.recipients"
                @update="updateRSID(item.id, 'recipients', $event, item.request_item && item.request_item.pk)"
              />
            </template>
            <template v-slot:item.authors="{ props: { item } }">
              <RelatedCharacter
                :value="item.authors"
                @update="updateRSID(item.id, 'authors', $event, item.request_item && item.request_item.pk)"
              />
            </template>
            <template v-slot:item.included_trial="{ props: { item } }">
              <Disputed
                :value="item.included_trial"
                @update="updateRSID(item.id, 'included_trial', $event, item.request_item && item.request_item.pk)"
              />
            </template>
            <template v-slot:item.confident="{ props: { item } }">
              <SelectCheckBox
                ...
                @update="updateRSID(item.id, 'confident', $event.value, item.request_item && item.request_item.pk)"
                keyValue="value"
              />
            </template>
            <template v-slot:item.privileges="{ props: { item } }">
              <SelectLabel
                ...
                multiple
                @chooseLabel="
                  updateRSID(item.id, 'privileges', $event, item.request_item && item.request_item.pk)
                "
              />
            </template>
            <template v-slot:item.responsive="{ props: { item } }">
              <SelectCheckBox
                ...
                @update="updateRSID(item.id, 'responsive', $event.value, item.request_item && item.request_item.pk)"
                keyValue="value"
              />
            </template>
            <template v-slot:item.desc="{ props: { item } }">
              <TextArea
                :value="item.desc"
                @update="updateRSID(item.id, 'desc', $event, item.request_item && item.request_item.pk)"
              />
            </template>
            <template v-slot:item.legal_issues="{ props: { item } }">
              <Issue
                :value="item.legal_issues"
                @update="updateRSID(item.id, 'legal_issues', $event, item.request_item && item.request_item.pk)"
              />
            </template>
            <template v-slot:item.reviewers="{ props: { item } }">
              <Members
                :value="item.reviewers"
                @update="updateRSID(item.id, 'reviewers', $event, item.request_item && item.request_item.pk)"
              />
            </template>
            <template v-slot:item.review_status="{ props: { item } }">
              <SelectCheckBox
                ...
                @update="updateRSID(item.id, 'review_status', $event.value, item.request_item && item.request_item.pk)"
                keyValue="value"
              />
            </template>
            <template v-slot:item.tags="{ props: { item } }">
              <Tags
                ...
                :isEmitOpenMenu="true"
                @update="updateRSID(item.id, 'tags', $event, item.request_item && item.request_item.pk)"
            ...
            <template v-slot:item.date_tagged="{ props: { item } }">
              <v-row no-gutters class="td-dateTag">
                ...
                <DateTagged
                  ...
                  "
                  class="mx-2"
                  @updateTime="updateTimeRSID(item.id, $event, item.request_item && item.request_item.pk)"
                />
              </v-row>
            </template>
            <template v-slot:item.labels="{ props: { item } }">
              <Label
                :value="item.labels"
                @update="
                  updateRSID(item.id, 'labels', $event, item.request_item && item.request_item.pk)
                "
                @onOpenMenu="
            ...
            <template v-slot:item.redaction="{ props: { item } }">
              <SelectCheckBox
                ...
                @update="updateRSID(item.id, 'redaction', $event.value, item.request_item && item.request_item.pk)"
                keyValue="value"
              />
            </template>
            <template v-slot:item.action="{ props: { item } }">
              <div class="d-flex">
                <v-btn
                  class="ma-0"
                  @click="updateBookmarkRSID(item.id, item.bookmarked_pk, item.request_item && item.request_item.pk)"
                  icon
                  small
                >
                ...
                <a
                  ...
                  :title="item.total_reports"
                  @click="updateReports(item.id, item.reported_object, item.request_item && item.request_item.pk)"
                >
                  <v-icon small :color="item.reported_object ? 'red' : 'grey'">mdi-flag</v-icon>
                </a>
                <Action
                  :item="item"
                  @remove="removeRSID(groupItem.groupID, item, $event, item.request_item && item.request_item.pk)"
                />
                <v-btn exact icon small @click="showDetail(item, groupItem.groupID)">
    ...
    <RequestSetDocumentDetail
      ...
      :itemsId="itemsId"
      @updateBookmarkRSID="updateBookmarkRSID"
      @remove="removeDetail"
...
import Comments from "../components/request-set-document/Comments";
...
import RequestSetDocumentDetail from "../components/request-set-documents-detail/index";
//form
import FormCreateRSID from "../components/request-set-document/FormCreateRSID";
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Request Number", value: "number", active: true },
  ...
  methods: {
    async removeDetail(row, close, itemsId) {
      await this.removeRSID(this.groupId, row, close, itemsId);
      this.showDetailRight = false;
    },
    async showDetail(item, groupId) {
      ...
      try {
        const idParams = {
          ...
          itemId: item.request_item.pk
        };
        await this.getDocumentDetailRequest(idParams);
    ...
    async removeRSID(name, row, close, itemsId) {
      const data = {
        name,
        matterId: this.$route.params.matter || this.$route.params.projectId,
    ...
    updateRSID(id, name, value, itemsId) {
      const data = {
        idParams: {
          matterId: this.$route.params.matter || this.$route.params.projectId,
          rSId: this.$route.params.requestSetId,
          itemsId: itemsId ? itemsId : this.itemsId,
          id
        },
        body: {
          [name]: value
    ...
    updateTimeRSID(id, value, itemsId) {
      const data = {
        idParams: {
          matterId: this.$route.params.matter || this.$route.params.projectId,
          rSId: this.$route.params.requestSetId,
    ...
    async updateReports(pk, value, itemsId) {
      const data = {
        idParams: {
          matterId: this.$route.params.matter || this.$route.params.projectId,
          rowId: pk,
    ...
    updateBookmarkRSID(id, bookmark, itemsId) {
      const data = {
        idParams: {
          matterId: this.$route.params.matter || this.$route.params.projectId,
          rSId: this.$route.params.requestSetId,
    ...
    async updateAttachmentRSID(id, value, itemsId) {
      const data = {
        idParams: {
          matterId: this.$route.params.matter || this.$route.params.projectId,
          rSId: this.$route.params.requestSetId,
