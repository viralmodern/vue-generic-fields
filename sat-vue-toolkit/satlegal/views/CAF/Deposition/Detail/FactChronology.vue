<template>
  <v-container fluid>
    ...
    >
      <v-expansion-panel
        style="width:100vh"
        v-for="(groupItem, i) in listGroupByCurrent"
        :key="i"
      >
        ...
        <v-expansion-panel-content>
          ...
          >
            ...
            <template v-slot:item.dispute="{ props: { item } }">
              <Disputed
                :value="item.dispute"
                @update="updateFacts(item.id, 'dispute', $event)"
              />
            </template>
            <template v-slot:item.important="{ props: { item } }">
              <Range
                :value="item.important"
                @update="updateFacts(item.id, 'important', $event)"
              />
            </template>
            <template v-slot:item.relevant="{ props: { item } }">
              <Range
                :value="item.relevant"
                @update="updateFacts(item.id, 'relevant', $event)"
              />
            </template>
            ...
            <template v-slot:item.interpretation="{ props: { item } }">
              <TextArea
                ...
                @update="updateFacts(item.id, 'interpretation', $event)"
              />
            </template>
            <template v-slot:item.subjects="{ props: { item } }">
              <Subject
                :value="item.subjects"
                @update="updateFacts(item.id, 'subjects', $event)"
              />
            </template>
            ...
            <template v-slot:item.sources="{ props: { item } }">
              <Source
                :value="item.sources"
                @update="updateFacts(item.id, 'sources', $event)"
              />
            </template>
            ...
            <template v-slot:item.action="{ props: { item } }">
              <Action
                :item="item"
                @remove="removeFacts(groupItem.groupID, item, $event)"
              />
            </template>
          </TableGrid>
...
import FactDetail from "@/sat-vue-toolkit/satlegal/views/CAF/Fact/components/detail2/FactDetail";
import { favorables, typeFacts, level, nullalbe } from "@/sat-vue-toolkit/satlegal/config";
// cells component
...
//form
import FormCreateFact from "../components/FormCreateFact";
...
export default {
  ...
  methods: {
    ...
    async removeFacts(name, row, close) {
      ...
      this.removeRelationInDepositionDetail(row.id);
      close();
    },
    removeRelationInDepositionDetail(objectId) {
      const contentTypeId =
        this.appConfigListContentType[this.appConfigConstants.FACT_RELATED] &&
        this.appConfigListContentType[this.appConfigConstants.FACT_RELATED].value;
      const data = {
        contentTypeId,
        objectId
      };
      this.removeRelationsDepositionDetail(data);
    },
    ...
    async getGroupByCurrent(group) {
      ...
      switch (group) {
        ...
        case "legal_issues":
          ...
          return groupByCurrent;
        case "favorable":
          groupByCurrent.list = [...this.appConfigs.favorableFact, nullalbe].map(x => {
            return { id: x.id, groupID: x.id, value: x.id, text: x.name };
          });
          return groupByCurrent;
        case "relevant_level":
    ...
    async getDataFacts(query, isFirstCall) {
      ...
      const listFactRelationDepostion = this.$_appConfigMixin_filterValueByContentType(
        this.appConfigConstants.FACT_RELATED,
        this.dataDepositionDetail.relations_info
      );
      const searchObject = {
        page: 1,
        ...
        pks: listFactRelationDepostion.map(x => x.pk).join(",") || 0,
        ...query
      };
    ...
    ...mapMutations("ht_store/matter/discovery/deposition/detail", ["removeRelationsDepositionDetail"]),
  },
  mounted() {
    const matterId = this.$route.params.matter || this.$route.params.projectId;
