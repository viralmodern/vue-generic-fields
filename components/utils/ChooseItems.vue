<template>
  ...
  >
    <template v-slot:activator="{ on }">
      <slot name="content" :selected="selected" :on="on">
        ...
        <v-card-text>
          ...
          <template v-else>
            <ChipType
              ...
              close
              class="ma-1"
              :text="`${selected[itemText]}`"
              @click-close="clearItem(selected[itemValue])"
            />
          </template>
        </v-card-text>
        <v-divider />
            ...
            <v-hover v-slot:default="{ hover }" :key="index1">
              <v-list-item @click="chooseItem" :value="cate">
                <template v-slot:default="{ active, toggle }">
                  ...
                  <v-list-item-icon v-if="listIcon">
                    <v-icon>{{ cate[itemIcon] }}</v-icon>
                  </v-list-item-icon>
                  <v-list-item-content>
                    <v-list-item-title>
                      {{ cate[itemText] }}
...
export default {
  ...
  watch: {
    selected(n) {
      ...
      // console.log('KHOACAVE2', n)
      this.$emit('input', n)
      if (n && this.closeOnContentClick) {
        // todo: closeOnContentClick
        this.uiShowMenu = false
      }
    },
    value(v) {
      console.log('KHOACAVE1', v)
    },
  },
}
</script>
