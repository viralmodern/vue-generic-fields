...
export default {
  ...
  mounted() {
    this.getDataClientMeetingAgendaRequest();
    this.getDataMemberRequest();
    this.getDataMatterRequest();
  },
  methods: {
    postCreateNewMeeting(data) {
      this.addClientMeetingAgendaRequest(data);
    ...
    ...mapActions("ht_store/workspace/client/detail/meeting", [
      "getDataClientMeetingAgendaRequest",
      "addClientMeetingAgendaRequest"
    ]),
  ...
  computed: {
    groupMonthYear() {
      const group = {};
      this.dataClientMeetingAgenda.forEach(x => {
        const YYYYMM = moment(x.date).format("YYYY-MM");
        (group[YYYYMM] = group[YYYYMM] || []).push(x);
      });
      return group;
    },
    ...mapGetters("ht_store/workspace/client/detail/meeting", [
