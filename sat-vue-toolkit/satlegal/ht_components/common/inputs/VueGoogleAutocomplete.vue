  ...
  methods: {
    ...
    onPlaceChanged() {
      ...
      if (place.address_components !== undefined) {
        ...
        for (const key in location) {
            location[key] = (place.address_components.find(x => x.types.includes(key)) || {}).long_name;
        }
        const data = {
          extra: this.autocompleteText,
    ...
    updateCoordinates(value) {
      ...
      if (!this.geolocation.geocoder)
        this.geolocation.geocoder = new window.google.maps.Geocoder();
    ...
    formatResult(place) {
      ...
      for (let i = 0; i < place.address_components.length; i++) {
        ...
        if (addressType === "administrative_area_level_1") {
          returnData["state"] = place.address_components[i]["long_name"];
          returnData["state_code"] = place.address_components[i]["short_name"];
        }
      }

      returnData["latitude"] = place.geometry.location.lat();
      returnData["longitude"] = place.geometry.location.lng();
      returnData["raw"] = place.formatted_address;
      return returnData;
    },
