<template>
  <div class="px-2">
    ...
    <div class="pa-2" v-for="stage in item.list" :key="stage.name">
      ...
      <SlickTable
        class="py-2"
        :totals="(dataCorrespondence[stage.name] || {}).totals"
        :items="(dataCorrespondence[stage.name] || {}).list"
      ...
      >
        <template slot="cell" slot-scope="props">
          <component
            ...
            "
            @remove="removeCorrespondence(stage.name, props.dataRow.id)"
          />
        </template>
        <template v-slot:top>
          <div>
...
import FormCreateCorrespondence from "../../dialogs/FormCreateCorrespondence";
import DropDown from "../../menus/DropDown";

import TdDelete from "./cells/TdDelete";
import TdAttachment from "./cells/table-correspondence/TdAttachment";
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("ht_store/matter/discovery/deposition/preparation/correspondence", [
      "dataCorrespondence"
    ]),
  ...
  methods: {
    ...
    addCorrespondence(data) {
      this.addCorrespondenceRequest({ name: this.stageAdd.name, data });
      this.isShowCreate = false;
    },
    updateCorrespondence(item, correspondenceId, name, value) {
      const data = {
        name: item,
        idParams: {
          correspondenceId
      ...
      this.updateCorrespondenceRequest(data);
    },
    removeCorrespondence(name, correspondenceId) {
      const data = {
        name,
        correspondenceId
      };
      this.removeCorrespondenceRequest(data);
    },
    updatePagination(stage) {
      if (this.panel === this.index) {
        ...
        this.getDataCorrespondenceRequest({ stage, searchObject });
      }
    },
    onEndDragRows(event) {
      const {
      ...
      // const fromTask = this.dataCorrespondence[fromListName].list.find(
      //   x => x.id === taskId
      // );
      // if (toListName) {
      //   const tasks = this.dataCorrespondence[toListName].list;
      //   if (!tasks.length) {
      ...
      //   drag.order = order;
      // } else {
      //   const tasks = this.dataCorrespondence[fromListName].list;
    ...
    ...mapMutations("ht_store/matter/discovery/deposition/preparation/correspondence", [
      "clearData"
    ]),
    ...mapActions("ht_store/matter/discovery/deposition/preparation/correspondence", [
      "getDataCorrespondenceRequest",
      "addCorrespondenceRequest",
      "updateCorrespondenceRequest",
      "removeCorrespondenceRequest"
    ])
  },
  mounted() {
