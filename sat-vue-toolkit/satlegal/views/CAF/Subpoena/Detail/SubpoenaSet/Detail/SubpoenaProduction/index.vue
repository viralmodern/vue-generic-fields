        ...
        <v-expansion-panel-content>
          ...
          >
            <template v-slot:item.name="{ props: { item } }">
              <TextArea
                :value="item.name"
                @update="updateSubpoenaProdcution(item, 'name', $event)"
              />
            </template>
            <template v-slot:item.index="{ props: { item } }">{{ item.subpoena && item.subpoena.pk}}</template>
            <template v-slot:item.members="{ props: { item } }">
              <Members
                :value="item.members"
                @update="updateSubpoenaProdcution(item, 'members', $event)"
              />
            </template>
            <template v-slot:item.priority="{ props: { item } }">
              <SelectCheckBox
                ...
                @update="updateSubpoenaProdcution(item, 'priority', $event.value)"
                keyValue="value"
              />
            </template>
            <template v-slot:item.response_status="{ props: { item } }">
              <SelectCheckBox
                ...
                @update="updateSubpoenaProdcution(item, 'response_status', $event.value)"
                keyValue="value"
              />
            </template>
            <template v-slot:item.request_status="{ props: { item } }">
              <SelectCheckBox
                ...
                @update="updateSubpoenaProdcution(item, 'request_status', $event.value)"
                keyValue="value"
              />
            </template>
            <template v-slot:item.under_oauth="{ props: { item } }">
              <Disputed
                ...
                @update="updateSubpoenaProdcution(item, 'under_oauth', $event)"
              />
            </template>
            <template v-slot:item.objection="{ props: { item } }">
              <Disputed
                :value="item.objection ? true : false"
                @update="updateSubpoenaProdcution(item, 'objection', $event)"
              />
            </template>
            <template v-slot:item.req_desc="{ props: { item } }">
              <TextArea
                :value="item.req_desc"
                @update="updateSubpoenaProdcution(item, 'req_desc', $event)"
              />
            </template>
            ...
            <template v-slot:item.deadline="{ props: { item } }">
              <v-row no-gutters class="td-date">
                ...
                "
                  ...
                  class="mx-2"
                  @updateTime="updateSubpoenaProdcution(item, 'deadline', $event.date)"
                />
              </v-row>
            </template>
            <template v-slot:item.privilege_invoke="{ props: { item } }">
              <Disputed
                ...
                @update="updateSubpoenaProdcution(item, 'privilege_invoke', $event)"
              />
            </template>
            <template v-slot:item.res_desc="{ props: { item } }">
              <TextArea
                :value="item.res_desc"
                @update="updateSubpoenaProdcution(item, 'res_desc', $event)"
              />
            </template>
            <template v-slot:item.total_notes="{ props: { item } }">
              <Notes :totalNotes="item.total_notes" :subpoenaId="subpoenaId" :rowId="item.pk" />
            </template>
            ...
            <template v-slot:item.action="{ props: { item } }">
              <div class="d-flex">
                <BookmarkOrFlag
                  ...
                  :endpoint="`/api/matters/${$$$matterID}/subpoenas/${subpoenaId}/productions/${item.pk}`"
                  routerMutation="ht_store/matter/subpoena/subpoenaProduction/patchRequestApi"
                  :pk="item.id"
...
export default {
  ...
  methods: {
    ...
    ...mapMutations("ht_store/matter/subpoena/subpoenaProduction", [
      "getDataAttachments"
    ]),
    ...
    updateSubpoenaProdcution(item, name, value) {
      const data = {
        idParams: {
          matterId: this.$route.params.matter || this.$route.params.projectId,
          rowId: item.pk,
