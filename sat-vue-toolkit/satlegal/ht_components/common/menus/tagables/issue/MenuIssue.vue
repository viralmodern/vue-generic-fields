  ...
  methods: {
    ...
    async update({ dataForm, closeUpdate }) {
      try {
        ...
        await this.updateIssuesRequest({
          matterId: this.$route.params.matter || this.$route.params.projectId,
          ...dataForm
        });
    ...
    removeLabel(id) {
      ...
      this.removeIssuesRequest({ matterId: this.$route.params.matter || this.$route.params.projectId, id })
    }
  }
};
</script>
