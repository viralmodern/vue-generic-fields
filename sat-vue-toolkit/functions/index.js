...
export function $$attrByProp({ type, prop }) {
  try {
    return type
  } catch (e) {
    return 'N/A'
...
export function $$fileType(val) {
    try {
        let _temp = val.split('.')
        return _temp[_temp.length - 1].toString().toUpperCase()
    } catch (e) {
        return val
    }
}

export function $$linkCountLabel(v) {
  try {
    return v >= 2 ? `${v} links` : v <= 0 ? '' : `${v} link`
  } catch (e) {
    return ''
  }
}
export function $$fileCountLabel(val) {
    try {
        return val >= 2 ? `${val} files` : val <= 0 ? '' : `${val} file`
    } catch (e) {
        return ''
    }
}
export const $$getCountLabelTag = (v) => {
  try {
    return v >= 2 ? `${v} labels` : v <= 0 ? '' : `${v} label`
  } catch (e) {
    return ''
  }
}
export function $$strNumFormat(val) {
    try {
...
export function $$totalNotes(val) {
    try {
        if (typeof val === "number") {
            let v  = new Intl.NumberFormat().format(val)
            let str = val <= 0 ? `` : val > 1 ? `${v} Notes` : `${v} Note`
            return `${str}`
        }
        return val
    } catch (e) {
...
export const getItemValueKey = (v) => {
...
export const csvRowsContent = (rows) => {
  let csvContent = "data:text/csv;charset=utf-8,";
  rows.forEach(function(arrData, i) {
    let row = ""
...
export const printCsvFile = (csvContent) => {
...
export const getRowDataString = () => {
  let desserts = [],
    headers = [],
    headerCsv = [],
    csv = [],
    csvStringRowsContent;
  try {
    let rows = document.querySelectorAll('.row-table-item')
    let elFields = document.getElementById('hidden-fields')
    rows = Array.from(rows)
    let attributes = JSON.parse(elFields.dataset.attributes)
    headers = attributes.map(attribute => {
      // let temp = {...attribute}
      let temp = {}
      temp.text = attribute.label
      temp.value = attribute.name
      temp.name = attribute.name
      temp.width = 50
      return temp
    })
    headerCsv = attributes.map(attribute => {
    ...
    for(let i = 0; i < rows.length; i++) {
      ...
      for(let j = 0; j < columns.length; j++) {
        ...
        let attributeName = column.dataset.column
        let cellTextContent = column.querySelector('.item--cell').textContent
        let newStr = cellTextContent.trim();
        temp[attributeName] = newStr
        rowCsv[j] = newStr
      }
      desserts.push(temp)
      csv.push(rowCsv)
  ...
  csvStringRowsContent = csvRowsContent(csv)
  return {
    desserts,
    headers,
    csv,
...
export const fileNamesContext = (path) => {
  try {
...
export const $$arrToString = (val) => {
  if(Array.isArray(val)) return val.join(', ')
  return ''
}
export const $$slugToString = (val) => {
  try {
    let str = val.split('-')
    return str.join(' ')
  }catch (e) {
...
export const $formatFullDateTime = (val) => {
  // moment().format('MMMM Do YYYY, h:mm:ss a'); // October 29th 2019, 4:27:00 pm
  // moment().format('dddd');                    // Tuesday
  // moment().format("MMM Do YY");               // Oct 29th 19
  // moment().format('YYYY [escaped] YYYY');     // 2019 escaped 2019
...
export const $$getBgColorTag = (tag) => {
  try {
    let colorList = tag.split('#')
    let color = `#${colorList[colorList.length - 1]}`
...
// TODO: handle attribute methods
export const $$getTextFromList = ({ val, items, itemText, itemValue }) => {
  try {
    let k = typeof val === "object" ? val[itemValue] : val
    let itemsByValue = keyBy(items, itemValue)
    return itemsByValue[k][itemText] || ""
  } catch (e) {
    return ""
  }
}
export const $$getRawAddress = (address) => {
  try {
    return address.raw || '-'
  } catch (e) {
    return '-'
...
export function $$rangeThisWeek(fm = 'YYYY-MM-DD') {
  return [
    moment().startOf('week').format(fm),
    moment().endOf('week').format(fm),
  ]
}
export function $$rangeThisMonth (fm = 'YYYY-MM-DD') {
  return [
    moment().startOf('month').format(fm),
    moment().endOf('month').format(fm),
  ]
}
export function $$toDay (fm = 'YYYY-MM-DD') {
  return [
    moment().format('YYYY-MM-DD')
  ]
}
export function $$totalTask(v) {
  try {
...
export function $$totalTaskServer (v) {
  try {
    return v.count
  } catch (e) {
    return 0
...
export function $$emptyResults (dataGroup) {
  try {
    let total = 0
...
export function $$keyRow (row) {
  try {
    return `row-item-${row.pk || row.country || row.id || row.year}`
  } catch (e) {
    return ""
  }
}
export function $$sendMessageToParent(response, type, isDev) {
  let baseParent = ''
    if (isDev) {
      baseParent = 'http://localhost:3000'
    } else {
      // baseParent = process.env.VUE_APP_API_HOST
      baseParent = 'https://satlegal.ebitc.com'
    }
    baseParent = 'http://localhost:3000'
  // todo: inside iframe
  try {
    //iframe
    type = type || "jwt:login:compiled"
...
export function $$onMessageResponse(callback) {
  let targetFrame = window.frames
  targetFrame.addEventListener("message", callback)
}

export function $$darkRake(color) {
  try {
