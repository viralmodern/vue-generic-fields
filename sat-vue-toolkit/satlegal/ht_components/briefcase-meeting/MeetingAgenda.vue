...
export default {
  ...
  mounted() {
    this.getDataBriefcaseMeetingAgendaRequest();
    this.getDataMemberRequest();
    this.getDataMatterRequest();
  },
  methods: {
    postCreateNewMeeting(data) {
      this.addBriefcaseMeetingAgendaRequest(data);
    ...
    ...mapActions("ht_store/matter/briefcase/meeting", [
      "getDataBriefcaseMeetingAgendaRequest",
      "addBriefcaseMeetingAgendaRequest"
    ]),
  ...
  computed: {
    groupMonthYear() {
      const group = {};
      this.dataBriefcaseMeetingAgenda.forEach(x => {
        const YYYYMM = moment(x.date).format("YYYY-MM");
        (group[YYYYMM] = group[YYYYMM] || []).push(x);
      });
      return group;
    },
    ...mapGetters("ht_store/matter/briefcase/meeting", ["dataBriefcaseMeetingAgenda"]),
