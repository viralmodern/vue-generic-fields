...
export default {
  ...
  computed: {
     ...
     */
    getWidth() {
      return (this.$store.state.GanttElastic.options.width).toString().replace('-',"")
    },
     ...
     */
    getViewBox() {
      return `0 0 ${Math.round(this.getWidth)} ${
        this.$store.state.GanttElastic.options.allVisibleTasksHeight
      }`;
    }
  }
};
