<template>
  <v-menu
    :close-on-content-click="false"
    transition="scale-transition"
    :nudge-width="290"
    offset-y
  >
    <template v-slot:activator="{ on: onMenu }">
      <v-tooltip v-if="isShowOpenMenu" bottom>
        <template v-slot:activator="{ on: onTooltip }">
          <AddIcon
            :on="{ ...onMenu, ...onTooltip }"
            @onClickIcon="$emit('onOpenMenu')"
            :colorIcon="valid ? 'red' : ''"
            :style="`${valid ? 'border-color:red' : ''}`"
            :icon="icon"
          />
        </template>
        <span>{{textTooltip}}</span>
      </v-tooltip>
    </template>
    <v-col cols="12" class="pa-0">
      <Sniper :isLoading="isLoadingGet || isLoadingUpdate" />
      <v-list style="width:100%">
        <div class="text-center py-2 title">{{ title }}</div>
        <v-divider></v-divider>
        <div class="ma-2">
          <v-text-field
            class="input-search"
            hide-details
            solo
            v-model="search"
            :label="labelSearch"
          ></v-text-field>
        </div>
        <div class="scroll-list">
          <div
            style="width: 326px;text-align: center;padding: 5px 25px;"
            v-if="!listClone.length"
          >
            <slot v-if="!!$scopedSlots.searchNoResult" name="searchNoResult"></slot>
            <div v-else>
              There is no
              <b>{{ name }}</b>
              matched with your search pattern. Please consider adding
              {{ name }} first. Click
              <router-link :to="to || ''">here</router-link>
              to {{ name }} add
            </div>
          </div>
          <div v-else v-for="item in listClone" :key="item.id" class="my-2">
            <v-list-item ripple @click.prevent="$emit('select', item)">
              <slot name="item" :item="item"></slot>
            </v-list-item>
          </div>
        </div>
      </v-list>
    </v-col>
  </v-menu>
</template>
<script>
import AddIcon from "../common/buttons/AddIcon";
import Sniper from "../common/loadings/Sniper";

export default {
  components: {
    AddIcon,
    Sniper
  },
  props: {
    list: Array,
    labelSearch: String,
    title: String,
    name: String,
    to: [Object, String],
    keySearch: String,
    icon: String,
    textTooltip: String,
    valid: Boolean,
    isLoadingGet: Boolean,
    isLoadingUpdate: Boolean,
    isShowOpenMenu: {
      type: Boolean,
      default: () => true
    }
  },
  data() {
    return {
      search: ""
    };
  },
  computed: {
    listClone() {
      const filter = this.list.filter(
        x =>
          (x[this.keySearch] || "")
            .toLowerCase()
            .indexOf(this.search.toLowerCase()) > -1
      );
      return filter;
    }
  }
};
</script>

<style scoped>
.scroll-list {
  max-height: 320px;
  overflow: auto;
}
.scroll-list::-webkit-scrollbar-track {
  -webkit-box-shadow: inset 0 0 6px rgba(0, 0, 0, 0.3);
  background-color: #f5f5f5;
}
.scroll-list::-webkit-scrollbar {
  width: 3px;
  background-color: #f5f5f5;
}
.scroll-list::-webkit-scrollbar-thumb {
  background-color: #3f51b5;
}
.input-search >>> .v-input__slot {
  min-height: 42px;
}
</style>
