...
export default {
  ...
  removeRequestItemInTask(state, data) {
    for (const key in state.dataRequestSetTask) {
      state.dataRequestSetTask[key].results = state.dataRequestSetTask[key].results.map(x => {
        if (x.pk === data.requestSetTaskId) {
          x.request_items = x.request_items.filter(r => r.pk !== data.requestItemId);
          return x;
        }
        return x;
      });
  ...
  getRequestSetTaskDetail(state, data) {
    state.dataRequestSetTaskDetail = data
  },
  updateDataRequestSetTaskDetail(state, { result }) {
    state.dataRequestSetTaskDetail = { ...state.dataRequestSetTaskDetail, ...result }
  },
  removeRequestItemInTaskDetail(state, data) {
    let request_items = state.dataRequestSetTaskDetail.request_items;
    request_items = request_items.filter(r => r.pk !== data.requestItemId);
    state.dataRequestSetTaskDetail.request_items = request_items
  },
};
