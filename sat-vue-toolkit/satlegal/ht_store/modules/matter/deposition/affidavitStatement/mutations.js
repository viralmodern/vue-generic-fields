...
export default {
  ...
  removeDataKeyPleading(state, { name, affId, isFieldArray }) {
    if (isFieldArray) {
      for (var key in state.dataKeyPleading) {
        state.dataKeyPleading[key].results = state.dataKeyPleading[
          key
        ].results.filter(x => x.id !== affId);
  ...
  setDataAffDetail(state, data) {
    state.dataAffDetail = data
  },
  updateDataKeyPleadingDetail(state, { result }) {
    state.dataAffDetail = { ...state.dataAffDetail, ...result }
  },
